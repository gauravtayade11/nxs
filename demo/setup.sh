...
setup() {
  ...
  echo -e "  nxs k8s debug --pod crash-loop-demo -n $BAD_NS"
  echo -e "  nxs k8s debug --pod image-pull-demo -n $BAD_NS"
  echo -e "  nxs k8s debug --pod oom-demo -n $BAD_NS"
  echo -e "  nxs k8s debug --pod pending-demo -n $BAD_NS"
  echo ""
  echo -e "  ${GREEN}# GOOD CASE tests${RESET}"
  echo -e "  nxs k8s debug --pod healthy-demo -n $GOOD_NS"
  echo -e "  nxs k8s status"
  echo ""
  echo -e "  ${YELLOW}# LOG FILE tests (all tools)${RESET}"
  echo -e "  cat demo/logs/devops/docker-fail.log | nxs devops analyze --stdin"
  echo -e "  cat demo/logs/sec/trivy-critical.log | nxs sec scan --stdin"
  echo -e "  cat demo/logs/net/tls-expired.log    | nxs net diagnose --stdin"
  echo -e "  cat demo/logs/db/postgres-too-many-conn.log | nxs db diagnose --stdin"
  echo ""
